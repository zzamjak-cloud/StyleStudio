...
- **4x4 = 패치 구도**: 베이스(A) 위에 오버레이(B) 패치 한 덩어리. B 영역 = 캔버스의 12.5%~87.5%(가장자리에서 반 셀 안쪽), 전환 밴드는 그 경계선에 중심을 두고 폭 ≤ 셀의 절반. → 분할 시 볼록 코너 4 + 엣지 8 + 풀 4. (오목 코너 없음 — UI에 한계 명시)
- **8x8 = 도넛 구도**: 위 패치에 더해 중앙 2x2셀(37.5%~62.5%)에 순수 A 구멍. 구멍 전환선은 31.25%/68.75% 라인(2·5행/열 셀의 중앙). → 볼록 코너 4 + 엣지 32(방향별 변형 포함) + 오목 코너 4 + 풀 20 + 순수 베이스 4 = 완전한 Rule Tile 키트.
