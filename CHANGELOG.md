...
### Added
- 타일맵(TILEMAP) 세션에 룰타일(Rule Tile) 모드를 추가했습니다 — 변형 세트와 별개로 지형 전환용 코너/엣지/오목 타일셋을 생성할 수 있습니다.
- 룰타일 모드에서 베이스/오버레이 지형 2필드 입력과, 4x4(패치 구도)·8x8(도넛 구도) 전용 프롬프트를 추가했습니다.
- 결과 뷰에 룰타일 모드 전용 셀 역할 뱃지(코너·엣지·오목·풀·베이스)를 추가했습니다.
- 미리보기 캔버스에서 룰타일 모드일 때 주변 셀을 검사해 알맞은 역할 타일을 자동 배치하는 오토타일 스탬프를 추가했습니다.
- 유니티 임포트 가이드에 룰타일 모드 전용 안내(셀 역할 표, Rule Tile 설정 절차, 8x8 세트 권장 문구)를 추가했습니다.

### Changed
- 타일맵 변형 모드 프롬프트의 랜덤성 규칙을 개정해 반복적인 디테일 배치 패턴이 나오지 않도록 강화했습니다.

## [0.4.22] - 2026-08-27

### Added
- 타일맵(TILEMAP) 세션을 추가했습니다 — 유니티 Tilemap용 손맵(hand-painted) 변형 타일 세트를 4x4(16타일)·8x8(64타일) 그리드로 생성합니다(비율 1:1·해상도 1K 고정).
- 생성된 타일 시트의 이음새 품질을 자동 평가하는 seam 점수(0~100)와 경고 뱃지를 추가했습니다.
- 불량 타일만 선택해 재생성하고 제안된 교체를 확정/취소할 수 있는 기능과, 특정 슬롯을 교체에서 보호하는 락(lock) 기능을 추가했습니다.
- 타일을 가상의 맵에 스탬프/랜덤 채우기로 배치해 이음새를 육안으로 확인하는 미리보기 캔버스를 추가했습니다.
- 타일 시트·개별 타일 PNG·유니티 임포트 가이드를 `~/Downloads/AI_Gen/{세션명}/tilemap_{timestamp}/`에 내보내는 기능을 추가했습니다.
