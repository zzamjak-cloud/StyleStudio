 ...
 * 2. 변형은 정규 텍스처의 **안쪽에만** 스와치의 다른 위치 크롭을 이식한다
 *    (`buildTextureVariants` → `patchGraft.graftInterior`) → 조건 (a). 모든 변형의 변
 *    픽셀이 정규 텍스처와 동일하므로, 어떤 두 변형이 이웃해도 접합부가 이어진다.
 *    이식은 최소오차 컷으로 갈아탄 뒤 그래디언트 도메인 막으로 톤을 맞추므로, 계약을
 *    지키면서도 테두리와 안쪽이 무늬 수준에서 이어진다(알파 평균이 아니다).
 *
 ...
 * v3: 변형 내부를 알파 램프가 아니라 **최소오차 컷 + 그래디언트 도메인 이식**으로 얹는다
 *     (`patchGraft`). v2의 램프는 두 텍스처의 평균이라 타일마다 흐릿한 사각 액자가 남고
 *     무늬가 테두리에서 끊겼다 — 계약은 지켜졌지만 "외곽과 안쪽이 이어지지 않는" 상태였다.
 */
export const VARIATION_COMPOSER_VERSION = 3;
