...
## 타블렛 필압은 지원하지 않는다 (의도적으로 뺐음)

한 번 구현했다가 **되돌렸다.** 브라우저에서 필압을 받으려면 `PointerEvent.pressure`가 필요하고, 그러려면 입력이 `pointerType === 'pen'`으로 들어와야 한다. 그런데 이 프로젝트의 사용자는 **와콤 타블렛을 쓰기 위해 Windows Ink(Windows 펜)를 꺼 둔다** — 켜면 와콤 드라이버와 충돌해 타블렛 사용 자체가 어려워지기 때문이다. Windows Ink가 꺼져 있으면 타블렛이 **마우스로 보고**되므로 `pressure`가 0.5 고정으로 들어오고 필압을 알 길이 없다.

즉 앱에서 고칠 수 있는 문제가 아니다. 필압을 다시 넣으려면 사용자에게 Windows Ink를 켜라고 요구해야 하는데 그건 타블렛 사용 환경을 망가뜨리는 거래다.

- Stage 바인딩도 **마우스/터치 그대로 둔다.** 포인터 이벤트로 통합했던 것은 오직 `pressure`를 받기 위해서였고, 필압이 없으면 얻는 게 없으면서 터치 동작만 바뀐다.
- 굵기는 `[` `]`와 버튼으로 조절한다.
