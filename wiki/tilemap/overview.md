...
> 모델과 품질은 이 원칙의 예외로 풀렸다. 모델은 **덕테이프 계열로 좁힌 드롭다운**, 품질은 **모델이 지원하는 티어 버튼**이 그대로 나온다 — 2.0과 2.5를 실제로 비교해 볼 수 있어야 하기 때문이다. 비율·해상도·룰타일 그리드는 여전히 고정이다.

| 값 | 고정 | 방어 이펙트 | UI |
|---|---|---|---|
| 이미지 모델 | 기본 `TILEMAP_FIXED_IMAGE_MODEL = 'openai/gpt-image-2'`(덕테이프). **계열 안에서는 선택 가능** | `isTilemapCompatibleModel(imageModel)`이 false면(=나노바나나 계열) 기본값으로 되돌림 | **덕테이프 계열 3종 드롭다운** + 주의 문구 |
| 이미지 품질 | 기본 `medium`. **선택 가능** | 없음(모델 변경 시 지원 티어 밖이면 공용 보정만) | 모델 티어 수에 따라 3열/5열 버튼 |
| 비율·해상도 | 1:1 · 1K | — | 없음 |
| 그리드 (룰타일) | `TILEMAP_RULETILE_GRID = '8x8'` | 룰타일인데 8x8이 아니면 되돌림 | 읽기 전용 표시 |

- **모델**: 타일맵은 프롬프트가 지정한 레이아웃을 정확히 지켜야 하는데(변형=균질 스와치 1장, 룰타일=머티리얼 시트 2패널) 나노바나나 계열은 이를 자주 무시해 사용할 수 없는 결과를 냈다. 그래서 **모델 전체를 잠그는 대신 계열로 좁힌다** — 부모가 `getTilemapImageModels()`(= `available` 중 `provider === 'openai'`, 현재 덕테이프·2.5 플레어·2.5 선버스트 3종)를 `availableModels`로 내려주고, `GeneratorSettings`가 다른 세션과 같은 드롭다운으로 렌더한다. 계열 안(2.0 ↔ 2.5)의 선택은 사용자 몫이라 방어 이펙트가 건드리지 않고, 히스토리 복원 등으로 나노바나나 계열이 들어올 때만 기본값으로 스냅한다. 드롭다운 아래에 2.5의 스케일·랜덤성 회귀 주의 문구를 함께 띄운다. OpenAI 키가 없으면 경고 문구만 띄운다 — 없으면 생성이 그냥 실패한다.
  > **2026-09-12에 gpt-image-2.5 선버스트로 잠깐 올렸다가 같은 날 gpt-image-2(덕테이프)로 되돌렸다.** 같은 gpt-image 계열이라 파라미터는 동일해 상위 호환처럼 보였지만, 변형 세트를 2.0과 2.5 선버스트로 각각 뽑아 비교하니 2.5에서 두 가지가 무너졌다:
  > 1. **재질 스케일 붕괴** — 참조 이미지를 줬는데도 같은 재질을 3~4배 확대한 것처럼 그렸다. 잔돌 디테일이 뭉개져 타일로 쓸 밀도가 나오지 않았다.
  > 2. **변형 랜덤성 붕괴** — 같은 형상(모서리 홈 등)이 8칸 전부 같은 위치에 반복됐다. 변형 세트의 존재 이유가 사라진다.
  >
  > 품질 티어를 올려도 해결되지 않는다(티어가 아니라 구도 해석의 차이). **2.5를 다시 후보로 올리려면 이 두 가지를 실제 생성물로 먼저 확인할 것.** 2.5 두 티어는 여전히 `available`이라 드롭다운에서 사용자가 직접 고를 수는 있다(`imageModels.ts`의 `TILEMAP_FIXED_IMAGE_MODEL` 주석 참조).
...
- **투명 배경 토글도 TILEMAP에는 없다** — `TRANSPARENT_BACKGROUND_CAPABLE_SESSIONS`에 `TILEMAP`이 없다(타일맵 알파는 AI가 아니라 합성기가 만든다, 위 "투명 지형" 절 참고).
...
| 품질 버튼이 안 보임 | 모델의 `supports.qualities`가 1종일 때만 숨는다(나노바나나 계열). 덕테이프 계열이면 TILEMAP에서도 보인다 |
| 타일맵 모델 목록에 나노바나나가 없음 | 의도된 동작 — `getTilemapImageModels()`가 `provider === 'openai'`만 반환한다 |
