...
- 모델 목록은 `types/constants.ts`의 `ANALYSIS_MODELS`, 기본값은 `DEFAULT_ANALYSIS_MODEL`(`gemini-3.8-flash`). API 키는 URL 쿼리스트링으로 전달, 로그에선 마스킹.
